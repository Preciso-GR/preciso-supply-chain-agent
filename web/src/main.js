...
const state = { tab: 'Chat', processing: false, operation: null, step: steps.length, files: [], messages: [], backend: { connected: false, extractor: {} }, error: '', liveResult: null, liveAnswer: '', graphReady: false, extractionPayload: null, artifact: null }
...
function renderSidebar() {
  const output = state.artifact ? `<a class="output-item" href="${escapeHTML(state.artifact.url)}" download="preciso_extract.json">${icon('file', 20)}<span><b>preciso_extract.json</b><small>Exact Claude payload · download</small></span></a>` : '<p class="sidebar-empty">No outputs yet.</p>'
...
async function runGraphBuild() {
  ...
  state.artifact = extraction.artifact || null
...
async function startGraphBuild() { state.processing = true; state.operation = 'build'; state.step = 0; state.error = ''; state.artifact = null; render(); try { await runGraphBuild() } catch (error) { state.processing = false; state.operation = null; state.error = error.message; render() } }
